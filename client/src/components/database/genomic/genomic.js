...
import { getScaffoldData, updateDatabaseData, handleGenomicExpDownload, handleGenomicInSilicoDownload } from "../../../redux/actions/database";
import PageStyle from '../pageStyle';
import GenomicInSilicoAdvanceFilter from './genomicInSilicoAdvanceFilter';
import GenomicExpValidateAdvanceFilter from './genomicExpValidateAdvanceFilter';
...
class GenomicInSilico extends React.Component {
  state = {
    loading: false,
  ...
  componentDidMount() {
    ...
    this.props.getScaffoldData().then(() => { this.setState({ cardLoadingStatus: false }); });
  }
  ...
  handleExpDownload = () => {
    const { repeatMotif,
      ...
      this.props.handleGenomicExpDownload(obj).then((response)=>{
  ...
  handleInSilicoDownload = () => {
    const { 
      ...
      this.props.handleGenomicInSilicoDownload(obj).then((response)=>{
        if (response.status === 200) {
          //extract file name from Content-Disposition header
          var filename = this.extractFileName(response.headers['content-disposition']);
          //invoke 'Save As' dialog
  ...
  render() {
    ...
    const { genomicInSilicoData, genomicExpData } = this.props.database;
          ...
          <Tabs defaultActiveKey="1" className='filterTabs' onChange={this.handleTabChange}>
            ...
            >
              {this.props.database.tabValue === "1" && 
                <Spin spinning={this.props.database.loadingData}>
                  <GenomicInSilicoAdvanceFilter tabChanged={this.state.tabChanged} />
                </Spin>
              }
              ...
              <Table columns={columns} dataSource={genomicInSilicoData} scroll={{ x: true}} ref={el => (this.inSilicoRef = el)} size="small" />
            </TabPane>
            <TabPane
              tab={
                <span>
            ...
            >
              {this.props.database.tabValue === "2" &&
                <Spin spinning={this.props.database.loadingData}>
                  <GenomicExpValidateAdvanceFilter tabChanged={this.state.tabChanged} />
                </Spin>
              }
              <div classname="printDownload" style={{ margin: '20px', float: 'right', position: 'relative', zIndex: '999' }}>
                <ReactToPrint
              ...
              <Table columns={expValidateColumns} dataSource={genomicExpData} rowKey="id" scroll={{ x: true}} ref={el => (this.expValidateRef = el)} size="small"/>
            </TabPane>
          </Tabs>
      </PageStyle>
    );
...
export default withRouter(connect(mapStateToProps, actionCreators)(GenomicInSilico));
