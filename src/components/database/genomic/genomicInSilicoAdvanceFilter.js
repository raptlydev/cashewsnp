...
import { getGenomicInSilicoData, updateDatabaseData } from "../../../redux/actions/database";
...
class GenomicInSilicoAdvanceFilter extends Component {
	constructor(props) {
		super(props);
	...
	applyFilter = () => {
		...
		this.props.getGenomicInSilicoData(obj).then(() => {
	...
	createDropdown = () =>{
		const { scaffoldData } = this.props.database;

		const scaffoldDropdown = scaffoldData && scaffoldData.sort((a,b)=> a - b).map((item)=>{
			return <Option value={item.Scaffold}>{item.Scaffold}</Option>
		});
...
const WrappedDynamicRule = Form.create({ name: 'advance_filter' })(GenomicInSilicoAdvanceFilter);
