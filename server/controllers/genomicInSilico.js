...
import GenomicInSilico from "../models/genomicInSilico.js";
import Scaffold from "../models/scaffold.js";
import Users from '../models/users.js';
import fs from 'fs';
import { Parser } from 'json2csv'
import path from "path";

export const getGenomicInSilico = async (req, res) => {
    let scaffoldResult;
    try {
        ...
        scaffoldResult = await GenomicInSilico.find(query);
...
export const getScaffolds = async (req, res) =>{
    try {
        const postMessages = await Scaffold.find();
...
export const downloadGenomicInSilico = async (req, res) => {
    let result;
    try {
        const scaffold = req.body.scaffoldIds || [];
        const repeat_motif = req.body.repeatMotif || "";
        ...
        result = await GenomicInSilico.find(query).then((result)=>{
            const csvFields = ['scaffold', 'repeat_type', 'repeat_motif', 'copy_no', 'repeat_size', 'forward_primer', 'reverse_primer', 'PCR_product_size', 'ta_oc'];
            const json2csvParser = new Parser({ csvFields });
            const csv = json2csvParser.parse(result);
            fs.writeFile('Genomic-In-Silico.csv', csv, function(err) {
                if (err) throw err;
