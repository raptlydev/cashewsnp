...
import GenicExp from "../models/genicExp.js";
import Users from '../models/users.js';
import fs from 'fs';
import { Parser } from 'json2csv'
import path from "path";

export const getGenicExp = async (req, res) => {
    let scaffoldResult;
    try {
        ...
        scaffoldResult = await GenicExp.find(query);
...
export const downloadGenicExp = async (req, res) => {
    let result;
    try {
        const repeat_motif = req.body.repeatMotif || "";
        ...
        result = await GenicExp.find(query);
        ...
        fs.writeFile('Genic-Experimetaly-Validated.csv', csv, function(err) {
            if (err) throw err;
            console.log('file saved');
        });
        var file = './Genic-Experimetaly-Validated.csv';
